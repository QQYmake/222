...
v3 M0—M10 全部完成，591 个测试通过（575 v2+v3 单元测试 + 16 集成测试）。
...
- M8: ConsolidationPipeline（W1-W6 + 水位清理）
- M9: ContextBuilder 适配 + TurnRunner 注入 MemoryPort + app.py v3 接线
- M10: 全量回归 + 向后兼容验证 + 真实 API 冒烟
v3 架构不变量（28 条）全部实现并验证。

### 回归测试与 Bug 修复（13 个 P0）

修复了 13 个 P0 bug 并通过 31 个回归测试 + 16 个集成测试验证：

1. 并发 @4 串记忆：`_run_query_path()` 改用 `read_recall_by_id()` 精确读取
2. `after_turn()` 全量追加 → 只追加本轮新增消息（按 turn_id 去重）
3. `.message.content` 属性名错误 → `.message_content`
4. `resolve()` 未检查 `_wake_only` → 添加 `trigger_type` 参数
5. TurnRunner → `after_turn()` 签名不匹配
6. 空 @e 导致种子记忆注入 → 区分 None 和 ""
7. γ 超时取消后台 task → `asyncio.shield` 让后台继续
8. dict 与 ChatMessage 契约错位 → `_get_message_content()` 兼容 dict
9. `recall_as_tool()` 签名漂移 → 添加 `turn_id` 参数
10. `MemoryRecallExecutor` 返回类型不一致
11. `run_wake_turn` 不使用 memory_port → 添加 recall + after_turn
12. 2am 全表清理数据丢失 → 水位机制 `clear_raw_up_to`/`clear_recall_up_to`
13. `recall.degraded` 属性不存在 → `recall.mode == "degraded"`；`RetrievalPipeline.execute()` 和 `_generate()` 中 `raw_messages[-1].content` 对 dict 崩溃 → 兼容 dict

### 真实 API 冒烟

使用 uv 锁定依赖环境完成真实 DeepSeek API 冒烟（MEMORY_ENABLED=true）：
- 基础聊天、多轮对话、新窗口模式、Outbox 查询、错误请求拒绝、tools 拒绝、API Key 不泄露
- MEMORY_ENABLED=false 向后兼容验证通过
- 后端放行门槛全部满足
