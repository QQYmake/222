...
from health_bridge.push_service import PushOutcome
from health_push import main
...
class TestOnceCommand(unittest.TestCase):
    def _run_once(self, outcome: PushOutcome) -> int:
        ...
        with patch("health_push.load_push_config", return_value=config), \
             patch("health_push.run_once", side_effect=fake_run_once):
            return main(["once"])
...
class TestDryRunCommand(unittest.TestCase):
    def test_dry_run_exit_0(self):
        ...
        with patch("health_push.load_push_config", return_value=config), \
             patch("health_push.run_once", return_value=PushOutcome.DRY_RUN):
    ...
    def test_dry_run_no_token_needed(self):
        ...
        with patch("health_push.load_push_config", side_effect=fake_load), \
             patch("health_push.run_once", return_value=PushOutcome.DRY_RUN):
...
class TestWatchCommand(unittest.TestCase):
    def test_watch_returns_run_watch_exit_code(self):
        ...
        for expected in (0, 2, 3):
            with patch("health_push.load_push_config", return_value=config), \
                 patch("health_push.run_watch", return_value=expected):
    ...
    def test_watch_passes_config(self):
        ...
        with patch("health_push.load_push_config", return_value=config), \
             patch("health_push.run_watch", side_effect=fake_run_watch):
            main(["watch"])
...
class TestSourceOverride(unittest.TestCase):
    def test_source_override_applied(self):
        ...
        with patch("health_push.load_push_config", return_value=original), \
             patch("health_push.run_once", side_effect=fake_run_once):
...
class TestConfigOption(unittest.TestCase):
    def test_config_passed_to_load_push_config(self):
        ...
        with patch("health_push.load_push_config", side_effect=fake_load), \
             patch("health_push.run_once", return_value=PushOutcome.UPLOADED):
...
class TestConfigError(unittest.TestCase):
    def test_value_error_exit_2(self):
        with patch("health_push.load_push_config",
